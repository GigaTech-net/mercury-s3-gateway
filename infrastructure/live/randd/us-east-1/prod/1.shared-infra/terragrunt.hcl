...
terraform {
  source = "git::git@github.com:GigaTech-net/terraform-AWS-shared-tfstate.git?ref=0.1.0"
}
...
inputs = {
  shared_infra_bucket = "${local.prefix}${local.account_name}-mercury-shared-terragrunt-terraform-state"
  aws_region          = local.aws_region
  shared_resources = {
    "vpc-alb" = {
      key = "${local.account_name}/${local.aws_region}/prod/2.vpc-alb/terraform.tfstate"
    }
  }
}